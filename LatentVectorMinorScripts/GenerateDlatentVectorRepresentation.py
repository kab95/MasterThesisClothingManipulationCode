...
def expand_seed(seeds, vector_size):
  ...
  for seed in seeds:
    ...
    for i in range(vector_size[0]):
        resultInd.append( rnd.randn(vector_size[1]) )
    if len(resultInd) == 1: resultInd = resultInd[0]
    result.append(np.array([np.array(resultInd)]))
...
def LoadImageFromVector(vecList, Gs, truncation_psi=0.5, saveBool=False, saveNameMod=""):
    ...
    if saveBool: amlgImg.save("D:\PythonProjectsDDrive\MasterThesisClothesFittingwGANs\GeneratedImages\VectorDlatentRepresentationModifier\\" + saveNameMod + ".png")  # datetime.datetime.now().timestamp().__str__().replace(".", "") + ".png")

    #amlgImg.show()


def generateDisplayVectors(curSeed, vecValRange, stepValue, Gs):
    ...
    # seeds = expand_seed(seedData, (14, 512))
    templateSeed = load_seed_latents(seedData)

    # for j in range(templateSeed[0].shape[-2]):
    #     for i in range(templateSeed[0].shape[-1]):
    #         seeds = np.copy(templateSeed)
    #
    #         #curSeed += vectorExpandLength #Creates unique seeds for all images
    #         for k in range(vectorExpandLength):
    #             seeds[k][0][j][i] = vecValRange * -1 + stepValue * k
    #         #generate_images(Gs, seeds, [f"{i}_{dat}" for dat in seedData], "VectorAttributeImages", truncation_psi=0.5)
    #         LoadImageFromVector(seeds, Gs, saveBool=True, saveNameMod=f"{j}_{i}_LoadedImage-{vectorExpandLength}_")

...
#findes latest snapshot for our project
def findlatestBuild(path,positionstart,positionEnd):
    ...
    secondHigestfile = ""
    for i in fileListInt:
        if i[0] >= secondHigest and i[0] != highestint:
            secondHigest = i[0]
            secondHigestfile= i[1]
    if path == "network-snapshot*":
        return highestfile 
    else:
        return secondHigestfile
...
def training_loop(
    ...
    resume_pkl              = "results/Retrain/00008-stylegan2-DeepFashon-8gpu-config-f/network-snapshot-000614.pkl",     # Network pickle to resume training from, None = train from scratch.
...
#         resume_pkl = "results/Transfer/00003-stylegan2-DeepFashion_Full_1024-8gpu-config-f/network-snapshot-002826.pkl"
        resume_pkl = findlatestBuild(findlatestBuild("results/Transfer/*",17,22)+"/network-snapshot*",86,92)
        print("-------------------------------"+resume_pkl+"--------------------------------------------- training")
...
#         if resume_pkl is None or resume_with_new_nets:
    ...
    print("hei du er ikke ferdig enda")
    # All done.
    summary_log.close()
    training_set.close()
